...
class TransactionController extends Controller
{
    ...
    public function index(Request $request, RfidTransactionReadModel $readModel)
    {
        ...
        $branchId = $user->isSuperAdmin()
            ...
            : $user->branch_id;
        abort_if(! $user->isSuperAdmin() && (! $branchId || ! $user->branch?->is_active), 403, 'An active branch assignment is required.');
        abort_if($user->isSuperAdmin() && $branchId && ! $branches->contains('id', $branchId), 403, 'The selected branch is not available.');
