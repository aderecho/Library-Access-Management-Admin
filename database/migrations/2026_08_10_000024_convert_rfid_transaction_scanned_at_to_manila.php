...
{
    private const BACKUP_TABLE = 'rfid_transaction_scan_time_backups';
    ...
    public function up(): void
    {
        ...
        match (DB::getDriverName()) {
            'pgsql' => DB::statement(<<<'SQL'
                ...
                SET scanned_at = (backup.scanned_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Manila'
                FROM rfid_transaction_scan_time_backups AS backup
                WHERE rt.id = backup.transaction_id
            SQL),
            'sqlite' => DB::statement(<<<'SQL'
                UPDATE rfid_transactions
                SET scanned_at = datetime(scanned_at, '+8 hours')
                WHERE id IN (SELECT transaction_id FROM rfid_transaction_scan_time_backups)
            SQL),
            'mysql', 'mariadb' => DB::statement(<<<'SQL'
                UPDATE rfid_transactions AS rt
                INNER JOIN rfid_transaction_scan_time_backups AS backup
            ...
            'sqlsrv' => DB::statement(<<<'SQL'
                ...
                INNER JOIN rfid_transaction_scan_time_backups AS backup
                    ON backup.transaction_id = rt.id
            SQL),
            default => throw new \RuntimeException('Unsupported database driver for RFID scan-time conversion.'),
        };
    }
    ...
    public function down(): void
    {
        ...
        // Keep the backup table and its original values permanently.
    }
};
